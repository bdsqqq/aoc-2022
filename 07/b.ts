...
const initialUnusedSpace = fullSpace - sizes.get("/");
const spaceNeeded = 30000000 - initialUnusedSpace;
...
sizes.forEach((size, i) => {
  if (size >= spaceNeeded) {
    possible.push({ dir: i, size: size });
  }
});

let smallest = { size: Infinity };
possible.forEach((possibility) => {
  if (possibility.size < smallest.size) {
