...
const possibleStarts = grid.flat().filter((c) => getLetterValue(c.char) === 1);
const end = grid.flat().find((c) => c.char === "E");

if (possibleStarts === undefined || end === undefined)
  throw new Error("No possible starts or end found");

// console.log(possibleStarts, end);
...
possibleStarts.forEach((start) => {
  ...
  while (!(queue.length === 0)) {
    ...
    if (char === "E") {
      ...
      routeLenghts.push(path.length - 1);
      break;
    }
...
console.log(routeLenghts.sort((a, b) => a - b)[0]);
console.log("done");
